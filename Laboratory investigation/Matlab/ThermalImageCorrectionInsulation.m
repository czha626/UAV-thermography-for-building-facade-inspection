function [ThermalCorrected,Scale] = ThermalImageCorrectionInsulation(RGB,Thermal)
...
tagCentre = [];
for i = 1:4
    tagCentre(i,:) = [mean(loc(:,1,i)),mean(loc(:,2,i)),i];
end

tagCentre_mean = [mean(tagCentre(:,1)),mean(tagCentre(:,2))];
...
for i = 1:4 % extract the four conner of Apriltag
    if tagCentre(i,1) > tagCentre_mean(1,1) && tagCentre(i,2) > tagCentre_mean(1,2)
        CornerPoints(1,:) = loc(1,:,tagCentre(i,3));
    elseif tagCentre(i,1) > tagCentre_mean(1,1) && tagCentre(i,2) < tagCentre_mean(1,2)
        CornerPoints(2,:) = loc(2,:,tagCentre(i,3));
    elseif tagCentre(i,1) < tagCentre_mean(1,1) && tagCentre(i,2) < tagCentre_mean(1,2)
        CornerPoints(3,:) = loc(3,:,tagCentre(i,3));
    elseif tagCentre(i,1) < tagCentre_mean(1,1) && tagCentre(i,2) > tagCentre_mean(1,2)
        CornerPoints(4,:) = loc(4,:,tagCentre(i,3));
    end
end
